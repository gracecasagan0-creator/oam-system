...
def employee_login():
    if request.method == "POST":
        ...
        if employee_id == EMPLOYEE["employee_id"] and password == EMPLOYEE["password"]:
            session.clear()
            session["employee"] = EMPLOYEE["employee_id"]

            emp = next((e for e in employees if e["id"] == EMPLOYEE["employee_id"]), None)
            session["employee_name"] = emp["name"] if emp else "Employee"
